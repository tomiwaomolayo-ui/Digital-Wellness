  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Biological Mass Index</h2>
        ...
        <div className="flex bg-slate-200/50 p-1.5 rounded-2xl border border-slate-200 shadow-sm">
           <button onClick={() => setTimeframe('daily')} className={`px-6 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${timeframe === 'daily' ? 'bg-white text-rose-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>Daily</button>
           <button onClick={() => setTimeframe('weekly')} className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${timeframe === 'weekly' ? 'bg-white text-rose-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>Weekly</button>
        </div>
      </div>
          ...
          <div className="bg-slate-950 p-10 rounded-[2.5rem] shadow-2xl border border-slate-800 relative overflow-hidden">
            <div className="flex justify-between items-center mb-10 relative z-10">
               <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-bold text-white tracking-wide uppercase">Sensor Signal Stream</h3>
               </div>
            ...
            <div className="h-80 w-full relative z-10">
               <ResponsiveContainer width="100%" height="100%">
                 <AreaChart data={filteredLogs}>
                   ...
                   <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{fill: '#475569', fontSize: 10, fontWeight: 600}} dy={10} />
          ...
          <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="p-8 bg-rose-50 rounded-[2rem] border border-rose-100">
                 <h5 className="font-bold text-rose-900 text-lg mb-2 uppercase tracking-tight">Metabolic Focus</h5>
                 <p className="text-sm text-rose-700 leading-relaxed font-semibold opacity-80">Consistent mass telemetry helps VitalityPulse AI identify metabolic fluctuations. Tracking daily mass correlates with 40% higher goal success rates.</p>
               </div>
               <div className="p-8 bg-emerald-50 rounded-[2rem] border border-emerald-100">
                 <h5 className="font-bold text-emerald-900 text-lg mb-2 uppercase tracking-tight">Ecosystem Health</h5>
                 <p className="text-sm text-emerald-700 leading-relaxed font-semibold opacity-80">Trajectory analysis suggests a positive bio-trend. Maintain high hydration levels to support your goal of {user.targetWeight}kg.</p>
               </div>
            </div>
          </div>
        </div>
        ...
        <aside className="space-y-8">
           <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden">
              ...
              <div className="space-y-6">
                 <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-3 px-1">Current Body Mass (kg)</label>
                    <div className="flex gap-2">
                       <input 
                         type="number" 
                         className="flex-1 min-w-0 p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-rose-100 font-bold text-lg text-slate-700 transition-all" 
                         placeholder="0.0"
                         value={currentInput} 
                 ...
                 <div className="pt-6 border-t border-slate-50">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-3 px-1">Update bio-target (kg)</label>
                    <div className="flex gap-2">
                       <input 
                         type="number" 
                         className="flex-1 min-w-0 p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-slate-700 font-bold text-lg transition-all focus:ring-4 focus:ring-slate-100" 
                         placeholder="0.0"
                         value={targetInput} 
                         onChange={e => setTargetInput(e.target.value)}
                       />
                       <button onClick={() => updateTargetWeight(parseFloat(targetInput))} className="px-5 py-4 bg-slate-900 text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:bg-black transition-all shrink-0">Apply</button>
                    </div>
                 </div>
              </div>
           </div>
           ...
           <div className="bg-gradient-to-br from-indigo-600 to-blue-600 p-10 rounded-[2.5rem] text-white shadow-2xl shadow-indigo-200 relative overflow-hidden group">
              <div className="relative z-10">
                <h4 className="text-xl font-bold mb-8 uppercase tracking-widest">Goal Status</h4>
                <div className="flex items-center justify-between mb-3">
                   <span className="text-[10px] font-bold opacity-70 uppercase tracking-[0.2em]">Efficiency to Target</span>
                ...
                <button className="w-full py-4 bg-white/10 rounded-2xl text-[10px] font-bold uppercase tracking-[0.3em] flex items-center justify-center gap-3 hover:bg-white/20 transition-all border border-white/10 active:scale-95">
